 ...
 * authenticated on every host (a plain container image, say) -- it needs nothing
...
function createGithubClient(opts) {
  ...
  /** The shape hub's GitHub view (web/static/app.js's renderGitHub())
   *  actually renders: user identity, recent repos, unread notifications --
   *  ported from the retired legacy monolith's /api/github/snapshot (the
   *  route hub's api-compat.js proxied to as `legacy: true` until this
   *  existed). Degrades to nulls/empty arrays rather than throwing, same
   *  fail-soft pattern as githubApi() itself (no token configured is a
   *  normal state, not an error). */
  async function getSnapshot() {
    const [user, repos, notifications] = await Promise.all([
      githubApi('/user'),
      githubApi('/user/repos?sort=updated&per_page=20'),
      githubApi('/notifications?per_page=20'),
    ]);
    return {
      user: user ? { login: user.login, followers: user.followers || 0 } : null,
      repos: (repos || []).map(r => ({
        fullName: r.full_name, name: r.name, description: r.description, isPrivate: !!r.private,
      })),
      notifications: (notifications || []).map(n => ({
        subject: n.subject && n.subject.title, repo: n.repository && n.repository.full_name,
      })),
    };
  }

  return { githubApi, runGhCommand, runGhArgs, fetchContributions, getContributions, getSnapshot };
}
