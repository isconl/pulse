...
function createDatesClient(opts) {
  ...
  /**
   * BM26082001 -- only Architect's own birthday gets the age-revealing
   * countdown treatment (top-level daysToNext/yearsTurning, and the
   * "turns X" milestone label). The milestone ITSELF still fires for every
   * birthday, self or not (row's own explicit instruction: milestone
   * reminders stay unchanged) -- only its label loses the age for a
   * non-self birthday, so sendDueReminders' 30/7/1/0-day cascade keeps
   * working off the same date/days unconditionally, it just doesn't say
   * whose age it's counting.
   */
  async function computeDates(now = new Date()) {
    const today = new Date(now); today.setHours(0, 0, 0, 0);
    const DAY = 86400000;
    const people = await readTSV(peopleFile).catch(() => []);
    const selfIds = new Set(people.filter(p => (p.IS_SELF || '').toLowerCase() === 'yes').map(p => p.ID));

    return (await readTSV(datesFile)).filter(r => /^\d{4}-\d{2}-\d{2}$/.test(r.DATE)).map(r => {
      ...
      const isBirthday = (r.KIND || '').toLowerCase() === 'birthday';
      const isSelfBirthday = isBirthday && selfIds.has(r.PERSON_ID);
      ...
      if ((r.RECURS || '').toLowerCase() === 'yearly') {
        ...
        const daysToNext = Math.round((next - today) / DAY);
        const yearsTurning = next.getFullYear() - base.getFullYear();
        if (!isBirthday || isSelfBirthday) {
          out.daysToNext = daysToNext;
          out.yearsTurning = yearsTurning;
        }
        out.milestones.push({
          label: !isBirthday ? `${yearsTurning} year${yearsTurning === 1 ? '' : 's'}`
            : isSelfBirthday ? `turns ${yearsTurning}` : 'birthday',
          date: out.nextOccurrence, days: daysToNext });
      }
      if (diff > 0) {
  ...
  async function listDates(now = new Date()) {
    const dates = await computeDates(now);
  ...
  async function addDate(p) {
    ...
    const row = { ID: `D${String(n).padStart(3, '0')}`, TITLE: clean(p.title), DATE: p.date,
      ...
      NOTE: clean(p.note), PERSON_ID: clean(p.personId) };
    await appendTSV(datesFile, row);
    auditLog.log('date_added', { id: row.ID, title: row.TITLE });
    return { success: true, id: row.ID };
  }

  async function deleteDate(id) {
    const removed = await rewriteTSV(datesFile, rows => rows.filter(r => r.ID !== id));
