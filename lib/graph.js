 ...
 *
 * COPIED into pulse verbatim from vault/lib/graph.js, 17 Aug 2026 (fix for
 * the calendar M365 sync gap in task-backlog.md), including the ECONNRESET/
 * ETIMEDOUT/ECONNREFUSED/EPIPE retry wrapper added the same day -- see
 * INC-002 in _handoff/INCIDENTS.md. Each engine pulls its own MSGRAPH_*
 * secrets from Bitwarden independently already (same pattern as github.js's
 * GITHUB_TOKEN); a shared package across separate deployable repos isn't
 * how this fleet is structured, so a copy is the correct move here, not a
 * shortcut -- keep both copies in sync if either one changes.
 */
...
function httpsRequestOnce(options, postData) {
...
// A raw socket reset/timeout/refusal mid-request, not an HTTP response --
// found live 17 Aug: a sync pass failed a whole collection
// (learning/financial-intelligence, a course lesson-folder pullFolder())
// with "read ECONNRESET" and no retry at all, because graphRequest()'s own
// retry loop below only re-sends on an HTTP status (429/503/509) it
// actually got back -- a connection that resets before any response exists
// has nothing for that loop to see, so httpsRequestOnce's rejection just
// propagated straight up through pullFolder to sync-loop.js's per-
// collection catch and surfaced as a hard sync failure. These codes are
// exactly the transient, worth-a-retry class (a real DNS failure -
// ENOTFOUND - is deliberately NOT included here, since retrying a name
// that doesn't resolve is never going to succeed).
const TRANSIENT_NETWORK_ERRORS = new Set(['ECONNRESET', 'ETIMEDOUT', 'ECONNREFUSED', 'EPIPE']);
async function httpsRequest(options, postData, attempt = 1) {
  try {
    return await httpsRequestOnce(options, postData);
  } catch (e) {
    if (attempt < 3 && TRANSIENT_NETWORK_ERRORS.has(e.code)) {
      await new Promise(r => setTimeout(r, 400 * attempt));
      return httpsRequest(options, postData, attempt + 1);
    }
    throw e;
  }
}
