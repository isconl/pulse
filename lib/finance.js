...
const FINANCE_FILES = ['accounts.tsv', 'transactions.tsv', 'networth.tsv', 'goals.tsv', 'incomes.tsv', 'ventures.tsv', 'possessions.tsv'];
...
function computeSummary({ accounts, txs, snaps, goals, incomeStreams, allocationModelRaw, now = new Date(), defaultCurrency = '-' }) {
  ...
  return {
    currency: accounts.find(a => a.CURRENCY && a.CURRENCY !== '-')?.CURRENCY || defaultCurrency,
...
function createFinanceClient(opts) {
  const {
    ...
    // No hardcoded currency (was 'KES') -- money format is genuinely
    // tenant-specific, and '-' matches the TSV convention for "no value"
    // used everywhere else in this vault, rather than guessing a country.
    defaultCurrency = '-',
  } = opts;
  ...
  async function summary({ now } = {}) {
    return computeSummary({
      accounts: await readTSV('finance/accounts.tsv'),
      txs: await readTSV('finance/transactions.tsv'),
      snaps: await readTSV('finance/networth.tsv'),
      goals: await readTSV('finance/goals.tsv'),
      incomeStreams: await readTSV('finance/incomes.tsv'),
  ...
  async function upsertIncome(p) {
    const rows = await readTSV('finance/incomes.tsv');
    ...
    const row = {
      ...
      CURRENCY: (p.currency || existing?.CURRENCY || defaultCurrency).toUpperCase(),
    ...
    if (existing) await rewriteTSV('finance/incomes.tsv', rows2 => rows2.map(r => r.ID === id ? row : r));
    else await appendTSV('finance/incomes.tsv', row);
    auditLog.log('finance_income_upserted', { id, created: !existing });
    return { id, created: !existing, row };
  }

  async function upsertAccount(p) {
    const accounts = await readTSV('finance/accounts.tsv');
    ...
    if (existing) await rewriteTSV('finance/accounts.tsv', rows => rows.map(r => r.ID === id ? row : r));
    else await appendTSV('finance/accounts.tsv', row);
  ...
  async function addTransaction(p) {
    ...
    const seq = (await readTSV('finance/transactions.tsv')).filter(t => t.DATE === date).length + 1;
    const row = {
      ID: `finance-${date}-${String(seq).padStart(3, '0')}`,
      DATE: date, TYPE: p.type, AMOUNT: String(amount),
      CURRENCY: (p.currency || defaultCurrency).toUpperCase(),
    ...
    await appendTSV('finance/transactions.tsv', row);
  ...
  async function snapshot({ now } = {}) {
    ...
    const replaced = await rewriteTSV('finance/networth.tsv', rows => rows.filter(r => r.DATE !== today));
    await appendTSV('finance/networth.tsv', row);
  ...
  async function listVentures({ force = false } = {}) {
    const rows = await readTSV('finance/ventures.tsv');
    const out = [];
    for (const v of rows) {
  ...
  async function upsertVenture(p) {
    const rows = await readTSV('finance/ventures.tsv');
    const id = p.id || `ven-${slug(p.name, Date.now())}`;
    const existing = rows.find(r => r.ID === id);
    if (!existing && !String(p.name || '').trim()) throw new Error('name required');
    // BN26090610: previously dropped RENDER_URL/FOLDER/GITHUB/CATEGORY on
    // every call -- a full-row replace (not a merge) that silently wiped
    // those four columns the moment anyone called upsertVenture on an
    // EXISTING row (e.g. this row's own editable-UI "edit name" action),
    // even though the caller never touched them. Every field below now
    // follows the same "explicit undefined means leave alone" contract the
    // other five fields already had.
    const row = {
      ID: id,
      NAME: p.name !== undefined ? clean(p.name) : existing?.NAME,
      ...
      STATUS: p.status || existing?.STATUS || 'active',
      NOTE: p.note !== undefined ? (clean(p.note) || '-') : (existing?.NOTE || '-'),
      RENDER_URL: p.renderUrl !== undefined ? (clean(p.renderUrl) || '-') : (existing?.RENDER_URL || '-'),
      FOLDER: p.folder !== undefined ? (clean(p.folder) || '-') : (existing?.FOLDER || '-'),
      GITHUB: p.github !== undefined ? (clean(p.github) || '-') : (existing?.GITHUB || '-'),
      // CATEGORY is deliberately never guessed by this function -- per
      // Sconl's explicit instruction (BN26090610), it is written ONLY when
      // a caller (the editable UI) sends it explicitly. Left untouched on
      // every other field-only edit.
      CATEGORY: p.category !== undefined ? (clean(p.category) || '-') : (existing?.CATEGORY || '-'),
    };
    if (existing) await rewriteTSV('finance/ventures.tsv', rows2 => rows2.map(r => r.ID === id ? row : r));
    else await appendTSV('finance/ventures.tsv', row);
    venturesCache.delete(id);
    auditLog.log('venture_upserted', { id, created: !existing });
    // `success: true` alongside {id, created, row}: the webconsole's
    // finPost() helper (hub/web/static/app.js) checks response.success,
    // a convention this route inherited from the retired legacy monolith's
    // matching endpoint -- kept for that caller's sake, not because this
    // module needs it internally.
    return { success: true, id, created: !existing, row };
  }

  /** Discards a venture row entirely -- for a mis-mapped OneDrive discovery
   *  candidate, or any venture Sconl wants gone. Real delete (not a status
   *  flip) since a discovery candidate that turned out wrong has no reason
   *  to linger; rewriteTSV's own massacre guard still protects against a
   *  bulk-delete accident (refuses removing more than half the table in
   *  one call -- a single-row delete never trips it). */
  async function deleteVenture(id) {
    if (!id) throw new Error('which venture');
    const rows = await readTSV('finance/ventures.tsv');
    const existing = rows.find(r => r.ID === id);
    if (!existing) throw new Error(`No venture ${id}`);
    await rewriteTSV('finance/ventures.tsv', rows2 => rows2.filter(r => r.ID !== id));
    venturesCache.delete(id);
    auditLog.log('venture_deleted', { id, name: existing.NAME });
    return { success: true, id };
  }

  /**
   * Additive-only ingest for OneDrive-discovered venture candidates
   * (vault/lib/ventures-discovery.js's discoverVentures, pushed here via
   * POST /finance/ventures/discover-ingest -- BN26090610). Mirrors the
   * copy-only, never-overwrite discipline CLAUDE.md §2 already applies to
   * rclone/OneDrive data generally:
   *
   *  - A candidate whose FOLDER already appears on a real (non-placeholder)
   *    row is skipped outright -- already known, never touched, whatever
   *    Sconl has since set on it (CATEGORY included) stays exactly as-is.
   *  - A fully-empty placeholder row (every column literally "-", the
   *    unpopulated-schema state this whole row was scoped to fix) is reused
   *    in place rather than left dead alongside a freshly appended one.
   *  - Otherwise the candidate is appended as a brand-new row.
   *
   * CATEGORY is always written as "-" here -- deliberately never guessed,
   * left for Sconl to assign through the editable UI (see finance.js's own
   * upsertVenture comment on the same rule).
   */
  async function ingestDiscoveredVentures(candidates) {
    const rows = await readTSV('finance/ventures.tsv');
    ...
    for (const c of (candidates || [])) {
      const folder = clean(c.folder);
      const name = clean(c.name);
      if (!folder || !name) { skipped.push({ folder: c.folder || '', reason: 'missing folder/name' }); continue; }
      const dupe = working.find(r => r.FOLDER && r.FOLDER !== '-' && r.FOLDER === folder);
      if (dupe) { skipped.push({ folder, reason: 'already known', id: dupe.ID }); continue; }
      const emptySlot = working.find(r =>
        (!r.NAME || r.NAME === '-') && (!r.FOLDER || r.FOLDER === '-') && (!r.CATEGORY || r.CATEGORY === '-'));
      const id = (emptySlot && emptySlot.ID && emptySlot.ID !== '-') ? emptySlot.ID : `ven-${slug(name, `${Date.now()}-${n++}`)}`;
      const row = {
        ID: id, NAME: name, KIND: 'candidate', ANALYTICS_URL: '-', AUTH_SECRET: '-',
        STATUS: 'active', NOTE: 'Discovered from OneDrive _ace -- confirm/merge/discard, then assign a category.',
        RENDER_URL: '-', FOLDER: folder, GITHUB: '-', CATEGORY: '-',
      };
      working = emptySlot ? working.map(r => (r === emptySlot ? row : r)) : [...working, row];
      created.push({ id, folder, name });
    }
    if (created.length) await rewriteTSV('finance/ventures.tsv', () => working);
    auditLog.log('ventures_discovery_ingested', { created: created.length, skipped: skipped.length });
    return { created, skipped };
  }

  /**
   * BF26091401: Holdings' possessions registry -- a general asset registry
   * alongside the existing money-tracking data, not just money. First cut,
   * deliberately flat: no valuation-over-time tracking, no depreciation,
   * no categories UI beyond a free-text CATEGORY field -- same "improve it
   * as we go" posture as the photos-tooling row (BP26090501).
   */
  async function listPossessions() {
    return readTSV('finance/possessions.tsv');
  }

  async function upsertPossession(p) {
    const rows = await readTSV('finance/possessions.tsv');
    const id = p.id || `pos-${slug(p.item, Date.now())}`;
    const existing = rows.find(r => r.ID === id);
    if (!existing && !String(p.item || '').trim()) throw new Error('item required for a new possession');
    const row = {
      ID: id,
      ITEM: p.item !== undefined ? clean(p.item) : (existing?.ITEM || id),
      CATEGORY: p.category !== undefined ? (clean(p.category) || '-') : (existing?.CATEGORY || '-'),
      VALUE: p.value !== undefined ? String(parseFloat(p.value) || 0) : (existing?.VALUE || '0'),
      ACQUIRED_DATE: p.acquiredDate !== undefined ? (clean(p.acquiredDate) || '-') : (existing?.ACQUIRED_DATE || '-'),
      LOCATION: p.location !== undefined ? (clean(p.location) || '-') : (existing?.LOCATION || '-'),
      CONDITION: p.condition !== undefined ? (clean(p.condition) || '-') : (existing?.CONDITION || '-'),
      NOTES: p.notes !== undefined ? (clean(p.notes) || '-') : (existing?.NOTES || '-'),
    };
    if (existing) await rewriteTSV('finance/possessions.tsv', rows2 => rows2.map(r => r.ID === id ? row : r));
    else await appendTSV('finance/possessions.tsv', row);
    auditLog.log('finance_possession_upserted', { id, created: !existing });
    // success:true alongside {id, created, row}: the webconsole's finPost()
    // helper (hub/web/static/app.js) checks response.success -- same
    // convention upsertVenture's own return already follows.
    return { success: true, id, created: !existing, row };
  }

  async function deletePossession(id) {
    if (!id) throw new Error('which possession');
    const rows = await readTSV('finance/possessions.tsv');
    const existing = rows.find(r => r.ID === id);
    if (!existing) throw new Error(`No possession ${id}`);
    await rewriteTSV('finance/possessions.tsv', rows2 => rows2.filter(r => r.ID !== id));
    auditLog.log('finance_possession_deleted', { id, item: existing.ITEM });
    return { success: true, id };
  }

  return {
    summary, upsertIncome, upsertAccount, addTransaction, snapshot,
    listVentures, upsertVenture, deleteVenture, ingestDiscoveredVentures,
    listPossessions, upsertPossession, deletePossession,
