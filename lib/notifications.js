...
function createNotificationsClient(opts) {
  ...
  if (!readTSV || !appendTSV || !rewriteTSV) throw new Error('createNotificationsClient requires readTSV/appendTSV/rewriteTSV');

  /** Notice one fact. Returns true when it was new, false when already known. */
  async function notify({ source, kind, title, body = '', view = '', ref = '', severity = 'info', dedupeKey }) {
    try {
      const key = String(dedupeKey || `${source}:${kind}:${title}`).replace(/[\t\r\n]+/g, ' ').slice(0, 160);
      const rows = await readTSV(notifFile);
      if (rows.some(r => r.DEDUPE_KEY === key)) return false;
      await appendTSV(notifFile, {
  ...
  async function notificationSweep({ deep = false } = {}) {
    ...
    let raised = 0;
    const add = async (n) => { if (await notify(n)) raised++; };

    // -- TASKS (scope/tasks.tsv, via the shared vault store): overdue, and due today --
    try {
      for (const t of (await readTSV('scope/tasks.tsv')) || []) {
        if (t.STATUS === 'done' || !t.DUE_DATE || t.DUE_DATE === '-') continue;
        if (t.DUE_DATE < today) {
          await add({ source: 'tasks', kind: 'overdue', severity: 'high',
          ...
          await add({ source: 'tasks', kind: 'due-today', severity: 'medium',
    ...
    try {
      for (const e of (await fetchCalendarEvents()) || []) {
        ...
        await add({ source: 'calendar', kind: e.date === today ? 'today' : 'tomorrow',
    ...
    try {
      for (const d of (await fetchDates()) || []) {
        ...
        await add({ source: 'dates', kind: 'approaching', severity: days <= 1 ? 'medium' : 'info',
    ...
    try {
      ...
      const txs = await readTSV('finance/transactions.tsv');
      for (const s of (await readTSV('finance/incomes.tsv'))) {
        ...
        if (!landed) {
          await add({ source: 'finance', kind: 'income-late', severity: 'high',
    ...
    try {
      ...
      if (status && (status.status === 'offline' || status.error)) {
        await add({ source: 'vault', kind: 'sync-failing', severity: 'high',
    ...
    if (deep) {
      try {
        for (const i of (await fetchJiraIssues()) || []) {
          await add({ source: 'jira', kind: 'status', severity: 'info',
          ...
          if (i.duedate && i.duedate < today && !/done|closed|resolved/i.test(i.status || '')) {
            await add({ source: 'jira', kind: 'overdue', severity: 'high',
    ...
    if (deep && githubApi) {
      try {
        ...
        for (const n of (Array.isArray(list) ? list : [])) {
          await add({ source: 'github', kind: n.reason || 'notification', severity: 'info',
    ...
    if (deep && githubApi && wellspringRepo) {
      ...
      try {
        ...
        for (const c of (Array.isArray(commits) ? commits : [])) {
          ...
          await add({ source: 'wellspring', kind: mine ? 'commit-own' : 'commit',
      ...
      try {
        ...
        for (const p of (Array.isArray(prs) ? prs : [])) {
          ...
          await add({ source: 'wellspring', kind: `pr-${p.state}`,
  ...
  async function listNotifications({ limit = 100 } = {}) {
    return (await readTSV(notifFile)).reverse().slice(0, limit);
  }

  async function markSeen({ ids = [], all = false, status = 'seen' } = {}) {
    const idSet = new Set(ids);
    let count = 0;
    let updated;
    // rewriteTSV's fn(rows) must return the new rows synchronously, so the
    // mutation and the persistence happen in the same pass -- mutating rows
    // from a separate readTSV() call (the previous version of this function)
    // only worked by accident against the old local store, whose read
    // returned shared object references; the remote store returns fresh
    // objects parsed from an HTTP response, so that mutation would silently
    // go nowhere.
    await rewriteTSV(notifFile, (rows) => {
      updated = rows.map((r) => {
        if (all || idSet.has(r.ID)) { count++; return { ...r, STATUS: status, SEEN_AT: new Date().toISOString() }; }
        return r;
      });
      return updated;
    });
