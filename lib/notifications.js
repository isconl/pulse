 ...
 * refactor canvas under `pulse`): Tasks and Jira live in `scope`; vault-sync
 * status lives in `vault`. A split-out pulse can't reach into their process
 * directly, so each is an injected async fetcher called at sweep time -- a
 * poll, not a shared closure. Any fetcher left unset just contributes
 * nothing, so this module runs standalone today and gains those sources the
 * moment hub wires the real clients in.
 *
 * OUT OF SCOPE for now, same reasoning: calendar/dates are pulse's own
 * capability but haven't been ported yet (queued next) -- also injected
 * fetchers, so this module won't need reshaping when they land.
 */
...
function createNotificationsClient(opts) {
  ...
  async function notificationSweep({ deep = false } = {}) {
    ...
    // -- TASKS (scope, injected): overdue, and due today --
    try {
      for (const t of (await fetchTasks()) || []) {
    ...
    // -- CALENDAR (pulse, not yet ported -- injected for now): today/tomorrow --
    try {
    ...
    // -- IMPORTANT DATES (pulse, not yet ported -- injected for now): a week's warning --
    try {
