 ...
 * directly and only worked because pulse and vault happened to share one
 * host's filesystem (true in Docker Compose, false on a container-per-engine
 * PaaS, which the fleet was briefly deployed to). That was a KNOWN GAP flagged in this
...
function createStore({ baseUrl, getToken = () => '', requestImpl, auditLog = { log: () => {} } }) {
  ...
  async function read(collection) {
    ...
    if (r.status !== 200) {
      auditLog.log('pulse_vault_read_failed', { collection, status: r.status });
  ...
  async function append(collection, row) {
    ...
    if (r.status !== 200) {
      auditLog.log('pulse_vault_append_failed', { collection, status: r.status });
  ...
  async function rewrite(collection, fn) {
    ...
    if (r.status !== 200) {
      auditLog.log('pulse_vault_rewrite_failed', { collection, status: r.status });
      throw new Error(`vault rewrite ${collection} failed: HTTP ${r.status}`);
    }
    return r.data.removed;
  }

  // Mirrors read/rewrite above, for the non-TSV state (JSON/YAML) that
  // vault's /vault-raw/:collection now serves -- see that route's own
  // comment for why this exists (pulse's calendar/rhythm state used to be
  // a plain local file, which broke the moment pulse ran on a different
  // host than vault, same class of bug read/append/rewrite were built to
  // fix for TSV rows).
  async function rawRead(collection) {
    const r = await request('GET', `/vault-raw/${encodeURIComponent(collection)}`);
    if (r.status !== 200) {
      auditLog.log('pulse_vault_raw_read_failed', { collection, status: r.status });
      throw new Error(`vault raw read ${collection} failed: HTTP ${r.status}`);
    }
    return r.data.text || '';
  }

  async function rawWrite(collection, text, { force = false } = {}) {
    const r = await request('PUT', `/vault-raw/${encodeURIComponent(collection)}`, { text, force });
    if (r.status !== 200) {
      auditLog.log('pulse_vault_raw_write_failed', { collection, status: r.status });
      throw new Error(`vault raw write ${collection} failed: HTTP ${r.status} ${r.data && r.data.error || ''}`);
    }
    return r.data.ok;
  }

  return { read, append, rewrite, rawRead, rawWrite };
}
