...
function createCalendarClient(opts) {
  const {
    ...
    // BG26082005: async () => [{id,title,date,time,source,location}, ...],
    // already in this module's own event shape (vault's GET /google/calendar
    // returns that shape directly, one call per connected account, pre-merged
    // by the caller) -- unlike graphRequest (a raw path-based fetcher this
    // module builds its own query against), Google's multi-account nature
    // means account selection/fan-out belongs at the wiring layer (server.js),
    // not here. Unset degrades to no Google events, same as graphRequest unset.
    googleCalendarFetch = null,
  } = opts;
  ...
  async function listEvents() {
    ...
    let googleEvents = [];
    if (googleCalendarFetch) {
      try { googleEvents = await googleCalendarFetch() || []; } catch {}
    }
    return [...events, ...msEvents, ...googleEvents];
  }
